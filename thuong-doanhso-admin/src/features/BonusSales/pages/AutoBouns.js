    ...
    useEffect(() => {
        ...
        if (OrderInfo) {
            ...
            const newObj = oiItems && oiItems.length > 0 ? oiItems.map(product => {
                ...
                const Hoa_hong_arr = hoa_hong.filter(item => item.SubSourceID === product.ID).reduce((r, { Value, User, SubSourceID, ID }) => {
                ...
                const T_Hoa_hong_arr = Hoa_hong_arr.map(item => item.Value)
                    .reduce((prev, curr) => prev + curr, 0);
                const Doanh_so_arr = doanh_so.filter(item => item.OrderItemID === product.ID).reduce((r, { Value, User, OrderItemID, ID }) => {
                    var temp = r.find(o => o.User.ID === User.ID);
                    if (!temp) {
                ...
                const T_Doanh_so_arr = Doanh_so_arr.map(item => item.Value)
                    .reduce((prev, curr) => prev + curr, 0);

                const new_Hoa_hong = Hoa_hong_arr.map(item => ({
                    SubSourceID: item.SubSourceID,
                    User: item.User,
                    Value: Math.round(product.gia_tri_thanh_toan * (item.Value / T_Hoa_hong_arr)),
                }));
                const new_Doanh_so = Doanh_so_arr.map(item => ({
                    OrderItemID: item.OrderItemID,
                    User: item.User,
                    Value: Math.round(product.gia_tri_doanh_so * (item.Value / T_Doanh_so_arr)),
                }));
                return {
                    Product: product,
                ...
                return (
                    <Form>
                        <Table bordered responsive>
                            ...
                            <tbody>
                                {
                                    values && values.AutoBouns && values.AutoBouns.map((item, index) => (
                                        <tr key={index}>
                                            <td>
                                                <div>
                                                    ...
                                                    <div>GT_DS: {formatVND(item.Product.gia_tri_doanh_so)}</div>
                                                    <div>GT_TT: {formatVND(item.Product.gia_tri_thanh_toan)}</div>
                                                </div>
                                            </td>
                                            <td>
                                                <FieldArray
                                                        ...
                                                        item.Hoa_Hong && item.Hoa_Hong.map((sub, idx) => (
                                                            <div className="d-flex align-items-center my-3" key={idx}>
                                                                ...
                                                                <NumberFormat
                                                                    ...
                                                                    onBlur={handleBlur}
                                                                />
                                                                <div className="text-danger w-30px text-end cursor-pointer" onClick={() => arrayHelpers.remove(idx)}>Xóa</div>
                                                            </div>
                                                        ))
                                                    )} />
                                            </td>
                        ...
                        <div>
                            <button className={`btn btn-success ${loading.autoBouns ? "spinner spinner-white spinner-right" : ""}`} type="submit" disabled={loading.autoBouns}>Cập nhập</button>
                        </div>
                    </Form>
                );
            }}
